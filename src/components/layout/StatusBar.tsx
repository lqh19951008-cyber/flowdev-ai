...
export function StatusBar({ showMiniMap = true, onToggleMiniMap }: StatusBarProps) {
  ...
  return (
    <footer className="h-7 w-full border-t border-slate-800/80 bg-slate-950/95 px-3 flex items-center justify-between text-[11px] text-slate-400 z-20 select-none shrink-0 backdrop-blur">
      {/* Left: Backend & Engine Status */}
      <div className="flex items-center gap-3">
        {/* Backend health */}
        <div className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
          <span className="font-mono text-slate-300">FastAPI :8000</span>
          <span className="text-emerald-400 font-medium">就绪</span>
        </div>
      ...
      <div className="flex items-center gap-3">
        {activeViewMode === "pipeline" ? (
          <>
            ...
            {/* Default LLM Model */}
            <div className="hidden lg:flex items-center gap-1 text-slate-400 font-mono text-[10px]">
          ...
          <div className="flex items-center gap-2 text-slate-400 font-mono text-[10px]">
            ...
            <span>个活跃项目仓库</span>
            <span className="text-slate-700">|</span>
            <span className="text-emerald-400">探针守护中</span>
          </div>
        )}
      </div>
    </footer>
