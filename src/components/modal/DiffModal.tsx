...
const DiffEditor = dynamic(
  () => import("@monaco-editor/react").then((mod) => mod.DiffEditor),
  {
    ssr: false,
    loading: () => (
      <div className="h-full w-full flex flex-col items-center justify-center bg-[#1e1e1e] text-slate-400 font-mono text-xs gap-2">
...
export function DiffModal() {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 select-none cursor-pointer"
      onClick={() => setDiffModalOpen(false)}
    >
      <div
        className="relative w-full max-w-6xl h-[90vh] rounded-2xl border border-slate-800 bg-slate-950 shadow-2xl flex flex-col overflow-hidden cursor-default"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="h-14 px-5 border-b border-slate-800 bg-slate-900/80 flex items-center justify-between shrink-0">
        ...
        <div className="h-14 px-5 border-t border-slate-800 bg-slate-900/80 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {activeTab === "diff" ? (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-300 hover:text-slate-100 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-lg transition-colors"
              >
